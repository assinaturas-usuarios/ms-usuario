...
class GlobalExceptionHandlerTest {
  ...
  @Nested
  @DisplayName("Exceções de negócio")
  class ExcecoesNegocio {
  ...
  @Nested
  @DisplayName("Exceções de validação e técnicas")
  class ExcecoesValidacaoTecnicas {
    ...
    void deveRetornar404ParaNoResourceFound() {
      NoResourceFoundException ex = new NoResourceFoundException(HttpMethod.GET, "/recurso");
      ProblemDetail result = handler.handleNoResourceFound(ex);
      assertThat(result.getStatus()).isEqualTo(HttpStatus.NOT_FOUND.value());
    }

    @Test
    @DisplayName("Deve retornar 503 para DataAccessException")
